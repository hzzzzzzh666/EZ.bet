<template>
  <el-container>
    ...
    <el-aside id="aside" width="250px">
      ...
      <el-menu :collapse-transition="false" :default-active="activeMenu" @select="handleSelect" router
        ...
        if (command === 'logout') {//退出
          ...
          this.$http.get(this.API.getCurrentUser).then((resp) => {
            if (resp.data.code === 200) {
              ...
              this.avatarUrl = this.currentUserInfo2.avatar ? this.ImageAPI + this.currentUserInfo2.avatar : null
            }
          })
        }
      },
