          ...
          <el-table :data="addExamQuestion" border style="margin-top: 10px">
            ...
            <el-table-column label="题库" width="155" align="center">
              <template slot-scope="scope">
                <el-select clearable v-model="scope.row.bankName" @change="handleChangeBank" placeholder="请选择题库"
  ...
  export default {
    ...
    methods: {
      ...
      handleChangeBank(bankId) {
        const target  = this.allBank.find(item => item.questionBank.bankId === bankId)
        console.log(target)
      },
      //添加题库组卷中的题库
      addBank () {
      ...
      //组卷模式变化
      makeModelChange () {
        this.$confirm('此操作将丢失当前组卷数据, 是否继续?', '提示', {
