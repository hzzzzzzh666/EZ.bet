<template>
  <el-container>
    ...
    <el-header>
      ...
      <el-button round type="primary" style="margin-left: 5px" icon="el-icon-plus" @click="showPublishNoticeDialog">发布新公告
      </el-button>
    </el-header>
    ...
    <el-main>
      ...
      <el-table
        ...
        <el-table-column align="center"
                         ...
                         label="操作">
          <template slot-scope="scope">
            <el-button round size="mini" type="warning" @click="showUpdateNoticeDialog(scope.row)">更新公告</el-button>
          </template>
        </el-table-column>
    ...
    <el-dialog title="发布公告" :visible.sync="publishTableVisible"  width="50%"
               @close="resetPublishForm"
               center>

      <el-form label-position="top" :model="publishForm" :rules="publishFormRules" ref="publishForm">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button round @click="publishTableVisible = false">取 消</el-button>
        <el-button round type="primary" @click="publishNotice">发 布</el-button>
      </div>
    </el-dialog>

    <el-dialog  title="更新公告" :visible.sync="updateTableVisible" width="50%"
               @close="$refs['updateForm'].resetFields()"
               center>

      <el-form label-position="top" :model="updateForm" :rules="updateFormRules" ref="updateForm">
    ...
    methods: { //获取用户信息
      ...
      selectChange (val) {
        ...
        if (val === 'delete') {//删除
          if (ids.length === 0) {
            ...
            this.$http.delete(this.API.deleteNotice, { params: { 'ids': ids.join(',') } }).then((resp) => {
              if (resp.data.code === 200) {
                //删除成功后,回调更新数据
                this.getNoticeInfo()
                this.$notify({
      ...
      updateNotice () {
        this.$refs['updateForm'].validate((valid) => {
          if (valid) {
            this.$http.put(this.API.updateNotice, this.updateForm).then((resp) => {
              if (resp.data.code === 200) {
                this.getNoticeInfo()
                this.$notify({
                  title: '提示',
