...
//            redisUtil.set("questionBanks", questionBanks, 60 * 5 + new Random().nextInt(5) * 60);
    ...
    public CommonResult<String> addQuestion(@RequestBody QuestionVo questionVo) {
        ...
        Integer currentQuId = qus.size() == 0 ? 1 : qus.get(qus.size() - 1).getId() + 1;
        ...
        if (redisUtil.get("questionVo:" + id) != null) {
            ...
            if (answer != null) {
                if (question.getQuType() != 2) {
                    ...
                    String imgsStr = answer.getImages();
                    ...
                    for (int i = 0; i < allOption.length; i++) {
                        ...
                        if (imgs.length > 0 && i <= imgs.length - 1 && !Objects.equals(imgs[i], ""))
...
//                    }
                    ...
                    for (int i = 0; i < allOption.length; i++) {
                        ...
                        answer1.setId(i);
                        answer1.setAnswer(allOption[i]);
                        if(imgsStr != null && !Objects.equals(imgsStr, "")) {
                            answer1.setImages(imgsStr.split( ","));
                        }
...
//            redisUtil.set("examRecord:" + recordId, examRecord, 60 * 5 + new Random().nextInt(2) * 60);
    ...
    public CommonResult<Object> getExamRecord(@RequestParam(required = false) Integer examId,
        ...
        examRecords.forEach(examRecord -> {
            ...
            examRecord.setUserName(user.getTrueName());
        });
        // 构造结果集
        Map<Object, Object> result = new HashMap<>();
