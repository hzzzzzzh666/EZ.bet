...
public class CommonController {
    ...
    public CommonResult<String> Register(@RequestBody User user) throws NoSuchAlgorithmException {
        ...
        //发放token令牌
        String token = TokenUtils.createToken(new TokenVo("1", user.getUsername(), newPwd,user.getAvatar(),user.getTrueName()));
    ...
    public CommonResult<String> login(@RequestParam(value = "username") String username,
        ...
        if (user != null) {
            ...
            if (newPwd.equals(user.getPassword()) && user.getStatus() == 1) {
                //发放token令牌
                String token = TokenUtils.createToken(new TokenVo(user.getRoleId() + "", user.getUsername(), newPwd,user.getAvatar(),user.getTrueName()));
